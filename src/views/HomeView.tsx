...
import { Product, Solution, Article, NavTab } from '../types';
import { PRODUCTS, SOLUTIONS, ARTICLES, CORE_VALUES } from '../data/mockData';
import { ProductCard } from '../components/ProductCard';
import { NewsCard } from '../components/NewsCard';
import { Sparkles, ArrowRight, ShieldCheck, Award, Cpu, Sprout, PhoneCall, ChevronRight, CheckCircle2 } from 'lucide-react';
    ...
    <div className="space-y-24 pb-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
            <button
              ...
              className="btn-gradient px-8 py-3.5 rounded-full text-sm font-semibold text-white shadow-xl shadow-[#FF007F]/25 flex items-center gap-2 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-8 py-3.5 rounded-full text-sm font-semibold text-white bg-white/5 hover:bg-white/10 border border-white/15 backdrop-blur-md transition-all flex items-center gap-2 cursor-pointer"
            >
          ...
          <div className="pt-12 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-md text-center">
              <div className="font-serif text-2xl sm:text-3xl font-bold text-white">15+</div>
              <p className="text-xs text-slate-400 mt-1">Năm Di Sản Bảo Tồn</p>
            </div>

            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-md text-center">
              <div className="font-serif text-2xl sm:text-3xl font-bold text-cyan-300">500+</div>
              <p className="text-xs text-slate-400 mt-1">Trang Trại Liên Kết</p>
            </div>

            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-md text-center">
              <div className="font-serif text-2xl sm:text-3xl font-bold text-[#ffb1c4]">99%</div>
              <p className="text-xs text-slate-400 mt-1">Dược Tính Giữ Nguyên</p>
            </div>

            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-md text-center">
              <div className="font-serif text-2xl sm:text-3xl font-bold text-emerald-400">100%</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <p className="text-sm text-slate-300 leading-relaxed">
                Tại VĐ VIỆT, mỗi cặp nhung hươu không chỉ là món quà quý giá từ thiên nhiên mà còn là kết tinh của quy trình chăn nuôi chuẩn sinh thái, sự tôn trọng sinh học động vật và ứng dụng công nghệ R&amp;D tiên tiến nhất.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                {CORE_VALUES.map((val) => (
                  <div key={val.id} className="p-4 rounded-xl bg-white/5 border border-white/5 space-y-2">
                    <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-[#ffb1c4]">
                      {val.iconName === 'ShieldCheck' && <ShieldCheck className="w-4 h-4" />}
                      {val.iconName === 'Cpu' && <Cpu className="w-4 h-4" />}
                      {val.iconName === 'Leaf' && <Sprout className="w-4 h-4" />}
                    </div>
                    <h4 className="font-semibold text-xs text-white">{val.title}</h4>
                    <p className="text-[11px] text-slate-400 leading-normal">{val.description}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="lg:col-span-5 relative">
              <div className="relative rounded-2xl overflow-hidden border border-white/15 shadow-2xl aspect-[4/3]">
                ...
                <div className="absolute bottom-4 left-4 right-4 p-3 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 text-xs">
                  <p className="font-semibold text-white">Trang Trại Hươu Sao Hương Sơn</p>
                  <p className="text-[11px] text-slate-300">Nơi khởi nguồn nguồn gen thuần chủng quý hiếm</p>
                </div>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
            VĐ VIỆT không chỉ cung cấp sản phẩm cao cấp mà còn hỗ trợ quy trình chuyển giao công nghệ chăn nuôi, lắp đặt thiết bị cảm biến và bao tiêu đầu ra toàn diện.
          </p>
        </div>
            ...
            >
              ...
              <div className="md:w-1/2 p-6 flex flex-col justify-between space-y-4">
                ...
                <button
                  ...
                  className="w-full btn-gradient py-2.5 rounded-xl text-xs font-semibold text-white flex items-center justify-center gap-2 cursor-pointer"
                >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {ARTICLES.map((art) => (
            <NewsCard key={art.id} article={art} onReadMore={onSelectArticle} />
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative rounded-3xl overflow-hidden p-8 sm:p-12 text-center bg-gradient-to-tr from-[#FF007F]/20 via-[#7928CA]/30 to-[#0D0B18] border border-white/20 shadow-2xl space-y-6">
          ...
          <div className="max-w-2xl mx-auto space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Hãy liên hệ ngay với chúng tôi để nhận mẫu dùng thử, thông tin kiểm định DNA và bảng báo giá chi tiết dành cho cá nhân hoặc trang trại đối tác.
            </p>
          </div>

          <div className="flex flex-wrap justify-center items-center gap-4 pt-2">
            <button
              onClick={() => onInquire('Tư vấn tổng thể sản phẩm')}
              className="btn-gradient px-8 py-3.5 rounded-full text-xs font-semibold text-white shadow-xl shadow-[#FF007F]/30 flex items-center gap-2 cursor-pointer"
            >
